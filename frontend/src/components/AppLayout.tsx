...
export function AppLayout() {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      senderId: 'peer',
      text: 'Hello, is this channel secure?',
      timestamp: new Date(),
      isSelf: false,
    },
    {
      id: '2',
      senderId: 'self',
      text: 'Yes, Double Ratchet is active.',
      timestamp: new Date(),
      isSelf: true,
    },
    {
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
      ...
      // Secure visual sanitization of the dropped file
      const safeName = sanitizeFilename(file.name);
      alert(`File dropped (sanitized): ${safeName}\nSize: ${formatSize(file.size)}`);
      // Trigger Phase 3 upload flow here...
    }
  };
        ...
        <div className="p-3">
          <div className="relative">
            ...
            <input 
              type="text" 
              placeholder="Search" 
              className="w-full bg-gray-200/50 dark:bg-gray-800/50 rounded-lg pl-9 pr-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-apple-blue"
            />
          </div>
        </div>
            ...
            <div key={msg.id} className={`flex ${msg.isSelf ? 'justify-end' : 'justify-start'}`}>
              <div 
                className={`max-w-[70%] rounded-2xl px-4 py-2 shadow-sm ${
                  msg.isSelf 
                    ? 'bg-apple-blue dark:bg-apple-darkBlue text-white rounded-br-none' 
                    : 'bg-white dark:bg-gray-800 border border-apple-border/50 dark:border-apple-darkBorder/50 rounded-bl-none'
                }`}
              >
                {msg.text && (
                  // Strictly rendered as text, preventing XSS
