...
describe('Double Ratchet', () => {
  it('should encrypt and decrypt sequentially', async () => {
    ...
    // Alice -> Bob
    const { header, ciphertext } = await DoubleRatchet.encrypt(aliceState, pt, AD);
    const decrypted = await DoubleRatchet.decrypt(bobState, header, ciphertext, AD);
  ...
  it('should handle out-of-order messages (Skipped Keys Window)', async () => {
    ...
    const msg1 = await DoubleRatchet.encrypt(aliceState, new TextEncoder().encode("Msg 1"), AD);
    const msg2 = await DoubleRatchet.encrypt(aliceState, new TextEncoder().encode("Msg 2"), AD);
    const msg3 = await DoubleRatchet.encrypt(aliceState, new TextEncoder().encode("Msg 3"), AD);

    // Bob receives msg 3 FIRST (out of order)
    const dec3 = await DoubleRatchet.decrypt(bobState, msg3.header, msg3.ciphertext, AD);
    expect(new TextDecoder().decode(dec3)).toBe("Msg 3");

    // Bob receives msg 1 LATER (skipped keys feature)
    const dec1 = await DoubleRatchet.decrypt(bobState, msg1.header, msg1.ciphertext, AD);
    expect(new TextDecoder().decode(dec1)).toBe("Msg 1");

    // Bob receives msg 2
    const dec2 = await DoubleRatchet.decrypt(bobState, msg2.header, msg2.ciphertext, AD);
